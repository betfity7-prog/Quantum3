...
async function restoreSession(sessionId) {
  try {
    ...
    if (hasAuthFiles) {
      ...
      const tracked = sessionTracker.getSession(sessionId);
      const phoneNumber = String(tracked?.phoneNumber || '').replace(/[^0-9]/g, '');

      await startBotForSession(sessionId, phoneNumber || '0000000000');
      return true;
    }
    return false;
  } catch (error) {
...
app.post('/api/pair', async (req, res) => {
  try {
    ...
    // Reuse active session for same phone to avoid duplicate sockets and invalidated pairing flow
    const existingSession = Array.from(sessionManager.pairingSessions.values()).find(s =>
      s.phoneNumber === cleanNumber && ['starting', 'connecting', 'waiting_qr', 'waiting_pairing'].includes(s.status)
    );
    ...
    if (existingSession) {
      return res.json({
        ...
        message: 'Existing pairing session reused'
      });
    }
...
server.listen(PORT, () => {
  ...
  const autoRestoreEnabled = String(process.env.ENABLE_AUTO_RESTORE || 'false').toLowerCase() === 'true';
  if (autoRestoreEnabled) {
    setTimeout(() => {
      autoRestoreSessions();
    }, 3000);
  } else {
    console.log(chalk.yellow('⏭️ Auto-restore disabled (set ENABLE_AUTO_RESTORE=true to enable)'));
  }
});
